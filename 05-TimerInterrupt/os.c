...
void init_systick(uint32_t period)
{
	...
	/* Set number of ticks between two systick interrupt*/
	period = HSI_VALUE * period - 1;
	if (period & 0xFF000000){
		putstr("systick load value bigger than 24 bit value\n");
	...
	SYSTICK->CTRL |= ENABLE_COUNTER |
					ENABLE_EXCEPTION |
					PROCESSOR_CLK_SOURCE;

}

void __attribute__((interrupt)) systick_handler(void)
{
	putstr("Interrupt from System Timer\n");
}

int main (void)
{
 	/* systick interrupt period is 1s*/
 	uint32_t period = 1;
