...
{
	putstr("User Task #1\n");
	putstr("now, return to kernel mode\n");
	syscall();
	putstr("user Task #2\n");
	putstr("now, return to kernel mode\n");
	syscall();
...
int main (void)
{
	...
	*GPIOA_PUPDR |= 0x05 << (2 * 9);
	...
	unsigned int usertask_stack[32];
	unsigned int *usertask_stack_start = usertask_stack + 32 -16;
	usertask_stack_start[8] = (unsigned int) &usertask; // same offset as lr in pop
	putstr(greet);
	putstr(" calling the user task\n");
	usertask_stack_start = activate(usertask_stack_start);
	putstr("Return to the OS mode!\n");
	putstr("calling user task again\n");
	usertask_stack_start = activate(usertask_stack_start);
	putstr("Return to the OS mode!\n");
	putstr("Going to OS infinite loop...\n");
	while(1);
